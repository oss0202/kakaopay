...
public class AccountRepositoryTest implements AccountRepository {
	...
	public long save(Account account) {
		...
		jdbcTemplate.update(connection -> {
			PreparedStatement ps = connection.prepareStatement("insert into accounts (email) values (?)",
	...
	public int update(Account account) {
		return jdbcTemplate.update("update accounts set email = ? where id = ?", account.getEmail(), account.getId());
	}
